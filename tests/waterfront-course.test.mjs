...
test('waterfront circuit is a continuous closed route on source street segments',()=>{
 ...
 assert.ok(Math.abs(length-d.course.length)<.02);assert.ok(length>2100&&length<2150);assert.ok(length/27>60,'even sustained top speed takes over a minute');assert.ok(length/19>110&&length/19<115);
 assert.ok(d.route.some(p=>p[0]===268.55&&p[1]===869.84),'passes King and 3rd before returning');
 assert.deepEqual(d.course.sections.map(s=>s.name),['South Park','2nd Street','Brannan Street','The Embarcadero','King Street','3rd Street']);
});

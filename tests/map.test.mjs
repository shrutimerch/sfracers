...
test('course follows the closed South Park street ring', () => {
  ...
  const length = data.route
    .slice(1)
    .reduce((n, p, i) => n + Math.hypot(p[0] - data.route[i][0], p[1] - data.route[i][1]), 0);
...
test('real buildings surround the starting street and extend above ground', () => {
  ...
  for (const building of data.buildings) {
    const distance = Math.min(
      ...building.points.map((p) => Math.hypot(p[0] - start[0], p[1] - start[1])),
    );
    ...
    assert.ok(
      Math.abs(geometry.boundingBox.max.y - building.height) < 0.01,
      'building must retain its mapped height above the road',
    );
    assert.ok(Math.abs(geometry.boundingBox.min.y) < 0.001);
    geometry.dispose();
  }

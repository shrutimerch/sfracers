...
test('Google Earth coordinates align with the South Park street route', () => {
  const matrix = earthToGame();
  assert.ok(ecef(37.786, -122.395).applyMatrix4(matrix).length() < 0.00001);
  const park = ecef(37.7816, -122.39395).applyMatrix4(matrix);
  assert.ok(Math.abs(park.x - 92.295) < 1);
  assert.ok(Math.abs(park.z - 489.28) < 2);
  const elevated = ecef(37.786, -122.395, 10).applyMatrix4(matrix);
  assert.ok(Math.abs(elevated.y - 10) < 0.00001);
});

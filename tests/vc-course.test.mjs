...
test('VC course closes on mapped streets and passes all three actual building frontages', () => {
  ...
  for (const [id, address, street] of [
    [1171034242, '380', 'Brannan Street'],
    [129176903, '180', 'Townsend Street'],
    [143294965, '600', 'Townsend Street'],
  ]) {
    const b = d.buildings.find((b) => b.id === id);
    assert.equal(b.address, address);
    assert.equal(b.street, street);
    const m = d.course.landmarks.find((m) => m.buildingId === id);
    assert.ok(m);
    const [px, pz] = m.position;
    let nearest = Infinity;
    for (let i = 1; i < d.route.length; i++) {
      const [ax, az] = d.route[i - 1],
        [bx, bz] = d.route[i];
      const dx = bx - ax,
        dz = bz - az,
        t = Math.max(0, Math.min(1, ((px - ax) * dx + (pz - az) * dz) / (dx * dx + dz * dz || 1)));
      nearest = Math.min(nearest, Math.hypot(px - ax - t * dx, pz - az - t * dz));
    }

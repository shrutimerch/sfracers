...
test('all facade types have a shipped PNG texture',()=>{
 ...
 assert.deepEqual([...kinds].sort(),['brick','glass','masonry']);
 for(const kind of kinds){const url=new URL(`../public/textures/${kind}-facade.png`,import.meta.url);assert.ok(statSync(url).size>10000);assert.equal(readFileSync(url).subarray(1,4).toString(),'PNG');}
});
test('facade UVs repeat at floor scale and roofs remain separate',()=>{
 for(const facade of ['brick','masonry','glass']){
  ...
  const {walls,roof}=buildingGeometry(building);walls.computeBoundingBox();roof.computeBoundingBox();
  ...
  assert.ok(Math.max(...walls.attributes.uv.array)>1,'facade image must repeat instead of stretching');
  assert.ok(Math.abs(walls.boundingBox.min.y)<.01);
  assert.ok(Math.abs(walls.boundingBox.max.y-building.height)<.01);
  assert.ok(Math.abs(roof.boundingBox.min.y-building.height)<.01);
  walls.dispose();roof.dispose();
 }
});

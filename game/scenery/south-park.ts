...
export function buildSouthPark(scene: T.Scene, d: MapData) {
  ...
  const concrete = mat('#b7b8ad'),
    edge = mat('#90968b'),
    steel = mat('#7c8583', 0.48),
    bark = mat('#777164'),
    wood = mat('#847354'),
    dark = mat('#343a39'),
  ...
  const rod = (a: T.Vector3, b: T.Vector3, r: number, m: T.Material, r2 = r) => {
    const delta = b.clone().sub(a),
      g = new T.CylinderGeometry(r2, r, delta.length(), 7);
    g.applyQuaternion(
  ...
  const slab = (pts: Point[], m: T.Material, y = 0.15) => {
  ...
  const strip = (pts: Point[], width: number, m: T.Material, y = 0.18) => {
    for (let i = 1; i < pts.length; i++) {
      const [x, z] = pts[i - 1],
        [xx, zz] = pts[i],
        len = Math.hypot(xx - x, zz - z);
      box((x + xx) / 2, y, (z + zz) / 2, len, 0.12, width, m, Math.atan2(zz - z, xx - x));
    }
  };
  const surface = (base: string, amount: number, repeat: number) => {
    ...
    for (let i = 0; i < 14000; i++) {
      const k = random();
      ctx.fillStyle = `rgba(${k > 0.5 ? '255,255,240' : '25,30,22'},${random() * amount})`;
  ...
  const parkTurf = parkGrass();
  textures.push(parkTurf.texture);
  const grass = parkTurf.material,
    gravel = surface('#b7ab91', 0.35, 1),
    asphalt = surface('#646965', 0.14, 18);
  bark.map = surface('#807a6c', 0.5, 2).map;
  // Ground UVs are in metres, preserving grain scale at driving height.
  const texturedSlab = (pts: Point[], m: T.Material, y: number) => {
    const shape = new T.Shape(pts.map(([x, z]) => new T.Vector2(x, -z)));
    const g = new T.ShapeGeometry(shape);
    g.rotateX(-Math.PI / 2);
    g.translate(0, y, 0);
  ...
  texturedSlab(park, grass, 0.13);
  strip(park, 0.34, edge, 0.22);
  ...
  for (const road of d.roads.filter((r) => r.name === 'South Park'))
    strip(road.points, road.width ?? 9.8, asphalt, 0.065);
  ...
  for (const road of d.roads.filter((r) => r.name === 'South Park' && r.points.length < 8)) {
    ...
    const total = pts
      .slice(1)
      .reduce((n, b, i) => n + Math.hypot(b[0] - pts[i][0], b[1] - pts[i][1]), 0);
    for (let i = 1; i < pts.length; i++) {
      const a = pts[i - 1],
        b = pts[i],
        len = Math.hypot(b[0] - a[0], b[1] - a[1]),
        angle = Math.atan2(b[1] - a[1], b[0] - a[0]);
      for (let u = 0.25; u < len; u += 0.5) {
        const along = travelled + u,
          nearPark = road.points[0][0] > 100 ? total - along : along;
        if (nearPark > 20) continue;
        for (let v = -4.65; v < 4.7; v += 0.26) {
          const stagger = Math.round(v / 0.26) % 2 ? 0.25 : 0;
          box(
            a[0] + Math.cos(angle) * (u + stagger) - Math.sin(angle) * v,
            0.146,
            a[1] + Math.sin(angle) * (u + stagger) + Math.cos(angle) * v,
            0.477,
            0.025,
            0.237,
            pavers[Math.abs(Math.round(u * 2 + v * 4)) % 4],
            angle,
          );
        }
      }
  ...
  for (const path of d.parkDetails?.paths || []) {
    ...
    if (path.id === 549848273) {
      ...
      for (let s = 0; s < len; s += 1.45) {
        const p = curve.getPointAt(s / len),
          v = curve.getTangentAt(s / len),
          angle = Math.atan2(v.z, v.x),
          w = 5.3 + Math.sin(s * 0.09) * 1.2;
        const sh = new T.Shape();
        const r = 0.68,
          h = w / 2;
        sh.moveTo(-r, -h + r);
        sh.lineTo(-r, h - r);
        sh.absarc(0, h - r, r, Math.PI, 0, true);
        sh.lineTo(r, -h + r);
        sh.absarc(0, -h + r, r, 0, -Math.PI, true);
        const g = new T.ShapeGeometry(sh, 8);
        g.rotateX(-Math.PI / 2);
        g.rotateY(-angle);
        g.translate(p.x, 0.26, p.z);
        add(g, concrete);
      }
    } else strip(path.points, path.sidewalk ? 2.1 : 3.3, concrete, 0.2);
  }
  // Park-local frame: long axis runs northeast, perpendicular axis across the green.
  const local = (u: number, v: number) =>
    new T.Vector3(90 + u * 0.704 + v * 0.71, 0, 490 - u * 0.71 + v * 0.704);
  for (const [u, v, rx, rz] of [
    ...
    for (let a = 0; a <= Math.PI * 2 + 0.01; a += Math.PI / 32) {
      const q = local(u + Math.cos(a) * rx, v + Math.sin(a) * rz);
      pts.push([q.x, q.z]);
    }
    texturedSlab(pts, gravel, 0.275);
    // Picnic tables with galvanized frames and adjacent round stools.
    for (const offset of [-3, 3]) {
      const q = p.clone().add(new T.Vector3(offset * 0.63, 0, -offset * 0.776));
      box(q.x, 0.86, q.z, 2.1, 0.1, 0.85, wood, -0.89);
      for (const s of [-1, 1]) {
        const seat = q.clone().add(new T.Vector3(s * 0.776 * 0.78, 0, s * 0.63 * 0.78));
        box(seat.x, 0.48, seat.z, 2.1, 0.1, 0.3, wood, -0.89);
        for (const end of [-0.75, 0.75]) {
          const leg = q.clone().add(new T.Vector3(end * 0.63, 0, -end * 0.776));
          rod(leg.clone().setY(0.2), leg.clone().setY(0.83), 0.045, steel);
        }
      }
    }
    for (let j = 0; j < 4; j++) {
      const q = local(u - 5 + j * 1.2, v + 4);
      rod(q.clone().setY(0.2), q.clone().setY(0.65), 0.055, steel);
      const g = new T.CylinderGeometry(0.25, 0.25, 0.045, 12);
      g.translate(q.x, 0.67, q.z);
      add(g, steel);
    }
  }
  // Low curved retaining walls and timber seat slats, never a tall fence.
  for (const [u, v, sign] of [
    [-32, -6, 1],
    [27, 6, -1],
  ]) {
    const pts: Point[] = [];
    for (let i = 0; i <= 24; i++) {
      const q = local(u - 12 + i, v + sign * Math.sin((i / 24) * Math.PI) * 2);
      pts.push([q.x, q.z]);
    }
    strip(pts, 0.55, concrete, 0.49);
    for (let i = 1; i < pts.length; i++) {
      const a = pts[i - 1],
        b = pts[i];
      box(
  ...
  for (const [index, point] of (d.parkDetails?.trees || []).entries()) {
    ...
    rod(base, top, 0.44 + random() * 0.17, bark, 0.23);
    const evergreen = index % 4 === 0 || index % 7 === 0;
    for (let j = 0; j < 6; j++) {
      const a = (j * Math.PI) / 3 + random(),
      ...
      rod(top, end, 0.19, bark, 0.055);
      for (let k = 0; k < 3; k++) {
        const tip = end
          .clone()
          .add(new T.Vector3((random() - 0.5) * 3, 1 + random() * 1.8, (random() - 0.5) * 3));
        rod(end, tip, 0.045, bark, 0.009);
        if (evergreen)
          for (let n = 0; n < 180; n++) {
            const az = random() * Math.PI * 2,
              cy = random() * 2 - 1,
              rr = Math.cbrt(random()),
              rad = Math.sqrt(1 - cy * cy) * rr;
            const g = new T.PlaneGeometry(0.24 + random() * 0.24, 0.11 + random() * 0.16);
            g.rotateX(random() * Math.PI);
            g.rotateY(random() * Math.PI * 2);
            g.rotateZ(random() * Math.PI);
            g.translate(
              tip.x + Math.cos(az) * rad * 1.65,
              tip.y + cy * rr * (evergreen ? 2 : 1.1),
              tip.z + Math.sin(az) * rad * 1.65,
            );
            add(g, leaves[(j + k + n) % 4]);
          }
        else
          for (let n = 0; n < 3; n++) {
            const twig = tip
              .clone()
              .add(new T.Vector3((random() - 0.5) * 1.6, 0.5 + random(), (random() - 0.5) * 1.6));
            rod(tip, twig, 0.015, bark, 0.004);
          }
      }
    }
  }
  // Shrubby planting pockets along the curb, with gaps at every mapped entrance.
  const entrances = (d.parkDetails?.paths || [])
    .filter((p) => !p.sidewalk)
    .flatMap((p) => [p.points[0], p.points.at(-1)!]);
  for (let i = 1; i < park.length; i++) {
    ...
    for (let s = 0; s < len; s += 1.2) {
      const x = a[0] + ((b[0] - a[0]) * s) / len,
        z = a[1] + ((b[1] - a[1]) * s) / len;
      if (entrances.some((p) => Math.hypot(p[0] - x, p[1] - z) < 3.4)) continue;
      if (random() < 0.65)
        ellipsoid(
          x + (90 - x) * 0.012,
          0.43,
  ...
  const footprint = feature.geometry
    .slice(0, -1)
    .map((g) => new T.Vector2((g.lon + 122.395) * 87900, (37.786 - g.lat) * 111200));
  const play = footprint
    .reduce((sum, p) => sum.add(p), new T.Vector2())
    .divideScalar(footprint.length);
  let xx = 0,
    zz = 0,
    xz = 0;
  ...
  const playAngle = 0.5 * Math.atan2(2 * xz, xx - zz),
    ca = Math.cos(playAngle),
    sa = Math.sin(playAngle);
  const us = footprint.map((p) => (p.x - play.x) * ca + (p.y - play.y) * sa),
    vs = footprint.map((p) => -(p.x - play.x) * sa + (p.y - play.y) * ca);
  const playLength = Math.max(...us) - Math.min(...us),
    playWidth = Math.max(...vs) - Math.min(...vs),
    net = mat('#666966');
  const playPoint = (u: number, y: number, v: number) =>
    new T.Vector3(play.x + ca * u - sa * v, y, play.y + sa * u + ca * v);
  ...
  for (const side of [-1, 1]) {
    ...
    for (let i = 0; i <= 40; i++) {
      ...
      ps.push(
        playPoint(
          (t - 0.5) * playLength,
          0.5 + Math.sin(t * Math.PI) * 3.4,
          side * playWidth * 0.4,
        ),
      );
    }
    add(new T.TubeGeometry(new T.CatmullRomCurve3(ps), 48, 0.09, 8, false), concrete);
  }
  for (let i = 0; i <= 15; i++) {
    const t = i / 15;
    rod(
      playPoint(
        (t - 0.5) * playLength * 0.95,
        0.6 + Math.sin(t * Math.PI) * 2.7,
        -playWidth * 0.35,
      ),
      playPoint((t - 0.5) * playLength * 0.95, 0.6 + Math.sin(t * Math.PI) * 2.7, playWidth * 0.35),
      0.024,
      net,
    );
  }
    ...
    for (let j = 0; j <= 20; j++) {
      ...
      ps.push(
        playPoint(
          (t - 0.5) * playLength * 0.95,
          0.6 + Math.sin(t * Math.PI) * 2.7,
          (-0.35 + (i * 0.7) / 6) * playWidth,
        ),
      );
    }
    add(new T.TubeGeometry(new T.CatmullRomCurve3(ps), 24, 0.022, 4, false), net);
  }
  const label = (
    text: string,
    x: number,
    ...
    const m = new T.Mesh(
      ...
      new T.MeshStandardMaterial({ map: t, roughness: 0.8, side: T.DoubleSide }),
    );
    m.position.set(x, y, z);
    m.rotation.y = -angle;
    scene.add(m);
  };
  for (const u of [-75, 75]) {
    const p = local(u, 0);
    rod(p.clone().setY(0.15), p.clone().setY(3.1), 0.055, steel);
    label('SOUTH PARK', p.x, 2.8, p.z, 2.8, -0.68);
    const q = local(u + 4, 1);
    box(q.x, 0.65, q.z, 0.5, 1, 0.5, steel);
  }
  ...
  for (const original of d.buildings.filter(
    ...
    const windowGlass =
      b.street === 'South Park' && b.address === '1' ? mat('#7f918c', 0.32) : glass;
    const id = b.id || 0,
      paint = mat(profile?.color || palette[id % palette.length]);
    ...
    const facing = b.points
      .slice(1)
      .map((p, i) => {
        const a = b.points[i],
          mid = new T.Vector2((a[0] + p[0]) / 2, (a[1] + p[1]) / 2);
        return {
          a,
          b: p,
          mid,
          len: Math.hypot(p[0] - a[0], p[1] - a[1]),
          score: mid.distanceTo(parkCenter),
        };
      })
      .filter((e) => e.len > 4)
      .sort((a, b) => a.score - b.score)
      .slice(0, b.street === 'South Park' && b.address === '1' ? 99 : 2);
    for (const wall of facing) {
      const { a, b: bb, len, mid } = wall;
      const angle = Math.atan2(bb[1] - a[1], bb[0] - a[0]);
      let nx = -Math.sin(angle),
        nz = Math.cos(angle);
      if ((mid.x - center.x) * nx + (mid.y - center.y) * nz < 0) {
        nx = -nx;
        nz = -nz;
      }
      const detail = (
      ...
      const trim = mat(profile?.trim || (id % 3 === 0 ? '#d1cbbc' : '#8c8e85'));
      const frameMat = profile ? mat(profile.frames) : dark;
      detail(len / 2, b.height - 0.12, len + 0.25, 0.3, 0.38, trim);
      detail(len / 2, b.address === '1' ? 8 : 3.5, len, 0.18, 0.23, trim);
      const bays =
          (b.street === 'South Park' && b.address === '1'
            ? Math.max(1, Math.round(len / 5.5))
            : profile?.bays) ||
          Math.max(1, Math.floor(len / (profile?.style === 'industrial' ? 4.2 : 3.1))),
        spacing = len / bays,
        floors = profile?.floors || Math.max(1, Math.round(b.height / 3.3));
      if (profile?.ground) detail(len / 2, 1.8, len, 3.6, 0.065, mat(profile.ground), 0.055);
      for (let floor = 0; floor < floors; floor++)
        for (let j = 0; j < bays; j++) {
          const u = (j + 0.5) * spacing,
            y =
              b.street === 'South Park' && b.address === '1'
                ? floor === 0
                  ? 2.7
                ...
                : floor === 0
                  ? 1.8
                  : (profile?.arches ? 6.2 : 4.9) +
                    ((floor - 1) * (b.height - (profile?.arches ? 7.8 : 5.5))) /
                      Math.max(1, floors - 1);
          if (y + 1 > b.height - 0.4) continue;
          const w =
              spacing *
              (id === 124889463
                ? 0.9
                : profile?.style === 'industrial'
                  ? 0.84
                  : floor === 0
                    ? 0.8
                    : 0.57),
                ...
                : floor === 0
                  ...
                  : id === 124889463
                    ...
                    : profile?.style === 'industrial'
                      ? 2.3
                      : 1.85;
          detail(u, y, w + 0.2, h + 0.22, 0.17, trim);
          detail(u, y, w, h, 0.08, windowGlass, 0.2);
          detail(u, y, 0.065, h, 0.11, frameMat, 0.26);
          detail(u, y + 0.12, w, 0.065, 0.12, frameMat, 0.26);
          detail(u, y - h / 2 - 0.13, w + 0.32, 0.15, 0.38, trim, 0.25);
          if (profile?.style === 'industrial') {
            for (let k = 1; k < 4; k++)
              detail(u - w / 2 + (w * k) / 4, y, 0.035, h, 0.12, frameMat, 0.29);
            for (let k = 1; k < 4; k++)
              detail(u, y - h / 2 + (h * k) / 4, w, 0.035, 0.12, frameMat, 0.29);
          }
          if (profile?.arches && floor === 0) {
            const arcY = y + h / 2;
            const pts: T.Vector3[] = [];
            for (let k = 0; k <= 20; k++) {
              ...
              pts.push(
                new T.Vector3(
                  a[0] + Math.cos(angle) * uu + nx * 0.28,
                  arcY + (Math.sin(t) * w) / 2,
                  a[1] + Math.sin(angle) * uu + nz * 0.28,
                ),
              );
            }
            add(new T.TubeGeometry(new T.CatmullRomCurve3(pts), 20, 0.07, 5, false), frameMat);
            const shape = new T.Shape();
            shape.absarc(0, 0, w / 2, 0, Math.PI, false);
            shape.lineTo(w / 2, 0);
            const g = new T.ShapeGeometry(shape, 16);
            g.rotateY(Math.atan2(nx, nz));
            g.translate(
              a[0] + Math.cos(angle) * u + nx * 0.2,
              arcY,
              a[1] + Math.sin(angle) * u + nz * 0.2,
            );
            add(g, windowGlass);
            if (b.address === '1') {
              const center = new T.Vector3(
                a[0] + Math.cos(angle) * u + nx * 0.28,
                arcY,
                a[1] + Math.sin(angle) * u + nz * 0.28,
              );
              for (const k of [3, 6, 10, 14, 17]) rod(center, pts[k], 0.035, frameMat);
            }
          }
        }
      if (b.street === 'South Park' && wall === facing[0]) {
        label(
          b.address || 'SOUTH PARK',
          mid.x + nx * 0.35,
          3.12,
          mid.y + nz * 0.35,
          Math.min(2.4, len * 0.4),
          Math.atan2(nx, nz) * -1,
          '#393f3e',
        );
      }
      // Brick coursing on brick warehouses: shallow mortar lines remain sharp close up.
      if (profile?.style === 'brick' || (!profile && (id % 4 === 0 || b.address === '164')))
        for (let y = 0.25; y < b.height - 0.4; y += 0.26)
          detail(len / 2, y, len, 0.018, 0.012, trim, 0.025);
      if (profile?.style === 'siding') {
        for (let y = 0.3; y < b.height - 0.5; y += 0.23)
          detail(len / 2, y, len, 0.018, 0.01, trim, 0.02);
        detail(len / 2, b.height - 0.45, len + 0.35, 0.18, 0.55, trim, 0.18);
      }
      if (profile?.awning) {
        detail(len / 2, 3.05, len * 0.54, 0.12, 1, frameMat, 0.5);
      }
      if (b.address === '135' && wall === facing[0]) {
        detail(len * 0.26, 1.6, len * 0.3, 2.8, 0.11, frameMat, 0.31);
        for (let k = 1; k < 6; k++)
          detail(len * 0.26, 0.2 + k * 0.46, len * 0.29, 0.04, 0.12, trim, 0.4);
      }
      if (b.address === '164') {
        const fins = mat('#784536');
        for (let u = 0.15; u < len; u += 0.42)
          detail(u, b.height / 2, 0.12, b.height, 0.5, fins, 0.4);
      }
    }
  }
  ...
  for (const car of parkParking(d.roads)) {
    ...
    box(x - Math.cos(ang) * 0.22, 1.18, z - Math.sin(ang) * 0.22, 2.2, 0.62, 1.5, glass, ang);
    box(x - Math.cos(ang) * 0.22, 1.53, z - Math.sin(ang) * 0.22, 2.2, 0.06, 1.53, body, ang);
    for (const dx of [-1.25, 1.25])
      for (const dz of [-0.84, 0.84]) {
        const q = new T.Vector3(
          x + Math.cos(ang) * dx - Math.sin(ang) * dz,
          0.42,
          z + Math.sin(ang) * dx + Math.cos(ang) * dz,
        );
        ellipsoid(q.x, q.y, q.z, 0.32, 0.32, 0.26, dark);
      }
    for (const side of [-1, 1]) {
      box(
        x + Math.cos(ang) * 2.11 - Math.sin(ang) * side * 0.58,
        0.75,
        z + Math.sin(ang) * 2.11 + Math.cos(ang) * side * 0.58,
        0.035,
        0.18,
        0.3,
        concrete,
  ...
  for (const [m, geoms] of groups) {
    ...
    mesh.castShadow = m !== grass && m !== asphalt && m !== gravel;

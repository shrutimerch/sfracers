import { finishBuildSteps, runBuildSteps } from '../loading/build-steps.ts';
...
import { buildRouteScenerySteps } from '../scenery/route-scenery';
import { hasRouteProfile } from '../scenery/config/route-profiles';
import { buildSouthParkSteps, isLocal } from '../scenery/south-park';
...
export function createWorld(canvas: HTMLCanvasElement, d: MapData, facades: Facades) {
  return finishBuildSteps(createWorldSteps(canvas, d, facades));
}
export function createWorldAsync(
  canvas: HTMLCanvasElement,
  d: MapData,
  facades: Facades,
  signal?: AbortSignal,
) {
  return runBuildSteps(createWorldSteps(canvas, d, facades), signal);
}
function* createWorldSteps(canvas: HTMLCanvasElement, d: MapData, facades: Facades) {
  ...
  try {
    ...
    if (d.coast?.length) {
      for (const coast of d.coast) {
        ...
        pts.forEach((p, i) =>
          i ? waterShape.lineTo(p[0], -p[1]) : waterShape.moveTo(p[0], -p[1]),
        );
    ...
    const disposeRoute = yield* buildRouteScenerySteps(scene, d, facades);
    const disposePark = yield* buildSouthParkSteps(scene, d);
    yield;

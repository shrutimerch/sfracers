...
export function streetSurface(hits: T.Intersection[], previous: number | null) {
  const surfaces = hits.filter((hit) => {
    ...
    return normal.y > 0.65 && (previous === null || Math.abs(hit.point.y - previous) < 2.5);
  });
  ...
  return {
    ground(x: number, z: number, previous: number | null) {
      ...
      for (const [dx, dz] of [
        ...
        ray.set(new T.Vector3(x + dx, previous === null ? 100 : previous + 3, z + dz), down);
        ray.near = 0;
        ray.far = previous === null ? 200 : 6;
        const height = streetSurface(ray.intersectObject(world, true), previous);
    ...
    keepClear(anchor: T.Vector3, desired: T.Vector3) {
      delta.subVectors(desired, anchor);
      const length = delta.length();
      if (length < 0.001) return desired;
      ray.set(anchor, delta.divideScalar(length));
      ray.near = 0;
      ray.far = length + 0.6;
      const hit = ray.intersectObject(world, true)[0];
      if (hit) desired.copy(anchor).addScaledVector(delta, Math.max(0, hit.distance - 0.65));
      return desired;
    },
  };
}

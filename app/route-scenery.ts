...
export function buildRouteScenery(scene:T.Scene,d:MapData,facades:Facades){
 ...
 for(let i=1;i<d.route.length;i++){const a=d.route[i-1],b=d.route[i],l=Math.hypot(b[0]-a[0],b[1]-a[1]),section=d.course?.sections.findLast(s=>s.start<=along+.1)?.name;along+=l;if(section!=='2nd Street'&&section!=='3rd Street')continue;const angle=Math.atan2(b[1]-a[1],b[0]-a[0]),offset=section==='2nd Street'?5.8:4.8,nx=-Math.sin(angle)*offset,nz=Math.cos(angle)*offset;line([[a[0]+nx,a[1]+nz],[b[0]+nx,b[1]+nz]],section==='2nd Street'?1.7:3,section==='2nd Street'?green:red,.102);}

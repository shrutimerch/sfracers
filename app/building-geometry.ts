...
export function buildingGeometry(building: Building) {
  ...
  for (let i = 1; i < building.points.length; i++) {
    const [ax, az] = building.points[i - 1],
      [bx, bz] = building.points[i];
    const length = Math.hypot(bx - ax, bz - az);
    if (length < 0.05) continue;
    const u = Math.max(0.5, Math.round(length / 4) / 2),
      v = Math.max(1, Math.round(building.height / 3.5)) / 2;
    position.push(
      ax,
      0,
      az,
  ...
  walls.setAttribute('position', new T.Float32BufferAttribute(position, 3));
  walls.setAttribute('uv', new T.Float32BufferAttribute(uv, 2));
  walls.computeVertexNormals();
  const shape = new T.Shape(building.points.map(([x, z]) => new T.Vector2(x, -z)));
  const roof = new T.ShapeGeometry(shape);
  roof.rotateX(-Math.PI / 2);
  roof.translate(0, building.height, 0);
  return { walls, roof };
}

...
export default function RaceGame({ variant = 'waterfront' }: { variant?: 'waterfront' | 'vc' }) {
  const vc = variant === 'vc';
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      import('./engine'),
      fetch(vc ? '/vc-course.json' : '/race-course.json').then((r) => {
    ...
    <main>
      <header>
        ...
        <span className="district">
          {vc ? 'SOMA · VC CIRCUIT' : 'SOUTH PARK → WATERFRONT'} <i>{vc ? '02' : '01'}</i>
        </span>
        <a className="route-link" href="/route">
          Compare routes
        </a>
      ...
      <section className="arena">
        ...
        <div className="race-title">
          ...
          <h2>{vc ? 'VC Circuit' : 'South Park Waterfront Circuit'}</h2>
          <p>
        ...
        {(ready || hud.mode === 'paused' || hud.mode === 'finished' || error) && (
          <div className="start-card">
              ...
              {error ||
                (hud.mode === 'finished'
                  ...
                  : hud.mode === 'paused'
                    ...
                    : vc
                      ? 'Pass South Park Commons, a16z and Pear VC on a real SoMa street loop. Look for the office markers. Same karts, a longer course.'
            ...
            <a className="variant-switch" href={vc ? '/' : '/vc'}>
              {vc ? 'Try A · Waterfront circuit →' : 'Try B · VC circuit →'}
            </a>
        ...
        <div className="minimap" hidden={hud.scenery?.startsWith('Google') || imageryLoading}>
          <span>
            {vc ? 'VC CIRCUIT' : 'WATERFRONT CIRCUIT'} <i>N ↑</i>
          </span>

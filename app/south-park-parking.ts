...
export function parkParking(roads:Road[]){
 ...
 for(const road of roads.filter(r=>r.name==='South Park')){const noseIn=isEastParkEntrance(road);let along=0;const total=road.points.slice(1).reduce((s,b,i)=>s+Math.hypot(b[0]-road.points[i][0],b[1]-road.points[i][1]),0);let next=8;
  for(let i=1;i<road.points.length;i++){const a=road.points[i-1],b=road.points[i],len=Math.hypot(b[0]-a[0],b[1]-a[1]),angle=Math.atan2(b[1]-a[1],b[0]-a[0]);
   while(next<along+len){const u=next-along;next+=noseIn?3:8.5;if(next>(total-5)||len<.01)continue;const x=a[0]+Math.cos(angle)*u,z=a[1]+Math.sin(angle)*u;
    for(const side of noseIn?[-1,1]:[1]){let nx=-Math.sin(angle)*side,nz=Math.cos(angle)*side;if(!noseIn&&(x-90)*nx+(z-490)*nz<0){nx=-nx;nz=-nz;}const offset=noseIn?5.05:3.5;cars.push({x:x+nx*offset,z:z+nz*offset,angle:noseIn?Math.atan2(nz,nx):angle,noseIn});}
   }along+=len;
  }
 }
 return cars;

 ...
 for(const road of d.roads){if(road.name!=='South Park'&&road.name!=='2nd Street')continue;
  ...
  for(let i=1;i<pts.length;i++){const a=pts[i-1],b=pts[i],len=Math.hypot(b[0]-a[0],b[1]-a[1]);if(len<.02)continue;const n0=normal(i-1),n1=normal(i),angle=Math.atan2(b[1]-a[1],b[0]-a[0]);
   ...
   for(let j=0;j<steps;j++)for(const side of [-1,1]){const t0=j/steps,t1=(j+1)/steps,tm=(t0+t1)/2,center=at(tm,side*(half+width/2));if(Math.hypot(center[0]-110,center[1]-490)>235)continue;
    ...
    const y0=height(inner0),y1=height(inner1);const m=concrete[(count++)%4];
    ...
    const v=new T.BufferGeometry();v.setAttribute('position',new T.Float32BufferAttribute([inner0[0],.06,inner0[1],inner1[0],.06,inner1[1],inner0[0],y0,inner0[1],inner1[0],.06,inner1[1],inner1[0],y1,inner1[1],inner0[0],y0,inner0[1]],3));v.computeVertexNormals();add(v,curb);
    const mid=at(tm,side*(half+.10));if(clear>3)box(mid[0],.287,mid[1],len/steps-.025,.014,.2,curb,angle);
    const seam=at(t0,side*(half+width/2));box(seam[0],y0+.006,seam[1],.022,.009,width,joint,angle);

   }
  }
 }
 for(const [m,geometries] of groups){const merged=mergeGeometries(geometries,false);if(!merged)continue;const mesh=new T.Mesh(merged,m);mesh.receiveShadow=true;mesh.castShadow=true;scene.add(mesh);geometries.forEach(g=>g.dispose());}
}
